...
resource "aws_customer_gateway" "cgw" {
  bgp_asn    = 65000
  ip_address = data.http.my_ip.body
  type       = "ipsec.1"
...
resource "aws_vpn_connection" "vpn" {
  ...
  static_routes_only = true # Si no usas BGP, utiliza rutas estáticas.
...
resource "aws_route_table" "autoscaling" {
  vpc_id = aws_vpc.main.id
  ...
  tags = {
    Name = "AutoScalingRouteTable"
  }
}

resource "aws_route_table_association" "asg_assoc" {
  subnet_id      = aws_subnet.private.id
  route_table_id = aws_route_table.autoscaling.id
}
...
resource "aws_autoscaling_group" "asg" {
  ...
  min_size            = 1
  desired_capacity    = 1
  vpc_zone_identifier = [aws_subnet.private.id] # Subred con la tabla de rutas asociada a la VPN
